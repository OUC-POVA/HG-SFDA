...
from hyperg.learning import tensor_hg_trans_infer
...
def train_hgat(model,optimizer,features,adj,labels):
    model.train()
    optimizer.zero_grad()
    
    g_ft,output,attention = model(features, adj)
    
    labels = labels.long().cuda()  
    output = output.cuda()
  
    ce_criterion = nn.CrossEntropyLoss().cuda()
    loss_ce = ce_criterion(output, labels) 
    
    mse_criterion = nn.MSELoss().cuda()
    loss_mr = mse_criterion(g_ft.cuda(), features.cuda())
    
    loss_train = 0.5 * loss_ce + 0.5 * loss_mr
...
def train_target_near1(args):
    ...
    oldC = ViTSFDA.VitSFDAClassifier(width=256, class_num=args.class_num, type=args.layer).cuda()
    ...
    optimizer = optim.SGD(
        [
            {"params": netF.base_network.parameters(), "lr": args.lr * 0.1},
            {"params": netF.bottleneck.parameters(), "lr": args.lr * 1} if netF.use_bottleneck else {},
            {"params": netF.classifier.parameters(), "lr": args.lr * 1},
            {"params": netF.sfda_bottleneck.parameters(), "lr": args.lr * 10.0},
            {"params": netF.sfda_bn.parameters(), "lr": args.lr * 10.0},
    ...
    #HGNN
    Embedding = hgnn.HGNN_GAT(args.class_num, 256)
    optimizer_c = optim.SGD(Embedding.parameters(), 0.001, momentum=0.9) 
    mse_criterion = nn.MSELoss().cuda()
    ce_criterion = nn.CrossEntropyLoss().cuda()
    ...
    hyper_interval = math.ceil(max_iter / args.hyper_iter)#max_iter // args.hyper_iter
    print("hyper_interval",hyper_interval)
    iter_num = 0
    j = 0
    loss_list = []
    acc_list = []
    ...
    while iter_num < max_iter:
        ...
        if iter_num == 1 or iter_num % hyper_interval == 0:
                ...
                for _ in range(20):
                  _, preds = torch.max(score_bank, 1)  
                  train_hgat(Embedding,optimizer_c,fea_bank, adj1,preds) #mem_label
                ...
                # Compute k-nearest neighbors for each sample in batch
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            # 
            print("training_time:", total_time / 60, "minutes")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hyper_iter", type=int, default=12)
    ...
    parser.add_argument("--output", type=str, default="VIToffice31_weight")  # trainingC_2
    ...
    parser.add_argument("--visda", action="store_true", default=False)
    parser.add_argument("--use_c", action="store_true", default=True)
    parser.add_argument('--beta1', default=0.8, type=float, help='ema for t')
    ...
    # parser.add_argument('--alpha', default=0.4, type=float, help='hoop')
